...
__fname = 'db_controller_ssh_attempt'
...
dbUser_prod = env.dbUser_prod # read_env()
...
#====================================================#
def open_database_connection_remote(use_ssh=False, ssh_dict={}, tunnel_port=None):
    ...
    try:
        ...
        if use_ssh: # NOTE_010524: not working correctly
            ...
            # import StringIO
            # import io
            # f = open(ssh_dict['path'],'r')
            # s = f.read()
            # # pass key to parmiko to get your pkey
            # pkey = paramiko.RSAKey.from_private_key(io.StringIO(s))
            ...
            # ssh_client.connect(ssh_dict['host'], username = ssh_dict['user'], pkey = pkey)
            ...
            # print('debug...')
            # print(pkey)
            # print(forward.getpeername())
            # print(forward.getpeername()[0])
            # print(forward.getpeername()[1])
            # print('debug... END')
            ...
            #     # tunnel = SSHTunnelForwarder((ssh_dict['host'], 22), ssh_password=SSH_PASS, ssh_username=SSH_UNAME,
            #         # remote_bind_address=(DB_HOST, 3306)) 
            #     # tunnel = SSHTunnelForwarder((ssh_dict['host'], 22), ssh_pkey=pkey, ssh_username=SSH_UNAME,
            #     #     remote_bind_address=(DB_HOST, 3306)) 
            ...
            # mypkey = paramiko.RSAKey.from_private_key_file(ssh_dict['path'])

            sql_hostname = dbHost
            sql_username = dbUser_prod
            sql_password = dbPw
            sql_main_database = dbName
            sql_port = 3306
            ssh_host = ssh_dict['host']
...
def exeMySqlDump(tableNames=None, fileName='nil_file.sql', use_remote=False, use_ssh=False, ssh_dict={}, tunnel_port=None):
    ...
    if use_remote:
        pass
        if open_database_connection_remote(use_ssh, ssh_dict, tunnel_port) < 0:
            return -1
    else:
    ...
    try:
        ...
        print('generating dump_command')
        if tableNames:
            # dump_command = f"mysqldump -u {dbUser_prod} -p -h {dbHost} --databases {dbName} --tables {tableNames}" # CLI enter password
            dump_command = f"mysqldump -u {dbUser_prod} -h {dbHost} --databases {dbName} --tables {tableNames}" # CLI enter password
        else:
            tableNames = 'ALL'
            # dump_command = f"mysqldump -u {dbUser} -p -h {dbHost} --databases {dbName}" # CLI enter password
            dump_command = f"mysqldump -u {dbUser_prod} -p -h {dbHost} --databases {dbName}" # CLI enter password

        print('generating dump_process')
        dump_process = subprocess.Popen(dump_command, stdout=subprocess.PIPE, shell=True)

        print('running dump_process.communicate() _ enter password ...')
        ...
        print(f"dump_command: '{dump_command}'")
        print(f"dump_error: '{dump_error}'")
        print()
        print(f"DONE: '{tableNames}' Tables  exported to {fileName}")
        print()
